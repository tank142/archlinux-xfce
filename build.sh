...
make_basefs() {
    ./mkarchiso.sh ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" init
    ./mkarchiso.sh ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "haveged intel-ucode amd-ucode memtest86+ mkinitcpio-nfs-utils nbd zsh efitools" install
}

# Additional packages (airootfs)
make_packages() {
    ./mkarchiso.sh ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.x86_64)" install
}
...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} ./mkarchiso.sh ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-lts -g /boot/archiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_customize_airootfs() {
    ...
    ./mkarchiso.sh ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs.sh' run
    rm ${work_dir}/x86_64/airootfs/root/customize_airootfs.sh
    #fix calamares
    ln -s /usr/lib/libboost_python3.so	${work_dir}/x86_64/airootfs/usr/lib/libboost_python3.so.1.66.0
    ln -s /usr/lib/libpython3.7m.so	${work_dir}/x86_64/airootfs/usr/lib/libpython3.6m.so.1.0
    ln -s /usr/lib/libpython3.7m.so	${work_dir}/x86_64/airootfs/usr/lib/libpython3.6m.so
    echo 'alias ls="ls --color=auto"
PS1="\[\e[01;31m\]\\u \[\e[01;37m\]\\w \$ \[\e[00m\]"' > ${work_dir}/x86_64/airootfs/root/.bashrc
}
...
make_boot() {
    ...
    cp -r ./Archxion ${work_dir}/iso/${install_dir}/boot/Archxion
}
...
make_syslinux() {
    ...
    cp /usr/lib/syslinux/bios/*.c32 ${work_dir}/iso/${install_dir}/boot/syslinux
    cp /usr/lib/syslinux/bios/lpxelinux.0 ${work_dir}/iso/${install_dir}/boot/syslinux
    cp /usr/lib/syslinux/bios/memdisk ${work_dir}/iso/${install_dir}/boot/syslinux
...
mkdir -p ${work_dir}/efiboot/EFI/
}

# Prepare efiboot.img::/EFI for "El Torito" EFI boot mode
make_efiboot() {
    cp "./grub.cfg" "${work_dir}/"
    sed -i "s/ARCHISO_LABEL/$iso_label/" "${work_dir}/grub.cfg"
    mkdir -p ${work_dir}/iso/EFI/archiso
    truncate -s 34M ${work_dir}/iso/EFI/archiso/efiboot.img
    mkfs.fat -n ARCHISO_EFI ${work_dir}/iso/EFI/archiso/efiboot.img
    mkdir -p ${work_dir}/efiboot
    mount ${work_dir}/iso/EFI/archiso/efiboot.img ${work_dir}/efiboot
    mkdir -p "${work_dir}/efiboot/EFI/boot"
    grub-mkstandalone -d /usr/lib/grub/x86_64-efi -O x86_64-efi --modules="part_gpt part_msdos" --fonts="unicode" --locales="ru" --themes="" -o "${work_dir}/efiboot/EFI/boot/bootx64.efi" "/boot/grub/grub.cfg=${work_dir}/grub.cfg" -v
    grub-mkstandalone -d /usr/lib/grub/i386-efi -O i386-efi --modules="part_gpt part_msdos" --fonts="unicode" --locales="ru" --themes="" -o "${work_dir}/efiboot/EFI/boot/bootia32.efi" "/boot/grub/grub.cfg=${work_dir}/grub.cfg" -v
    umount -d ${work_dir}/efiboot
    rm "${work_dir}/grub.cfg"
}

# Build airootfs filesystem image
make_prepare() {
    cp -a -l -f ${work_dir}/x86_64/airootfs ${work_dir}
    ./mkarchiso.sh ${verbose} -w "${work_dir}" -D "${install_dir}" pkglist
    ./mkarchiso.sh ${verbose} -w "${work_dir}" -D "${install_dir}" ${gpg_key:+-g ${gpg_key}} prepare
...
# Build ISO
make_iso() {
    ./mkarchiso.sh ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -P "${iso_publisher}" -A "${iso_application}" -o "${out_dir}" iso "${iso_name}-${iso_version}-x86_64.iso"
}
